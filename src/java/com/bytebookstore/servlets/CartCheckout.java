...
public class CartCheckout extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            int token = (Integer)request.getSession().getAttribute("token");
            ...
            try {
                ...
                query = "select email from USER where logkey_id=" + token + ";";
                st = conn.createStatement().executeQuery(query);
                
                if(st.next())
                    message.setRecipients(Message.RecipientType.TO, InternetAddress.parse(st.getString("email")));
                ...
                response.sendRedirect("/ByteBookstore/");
